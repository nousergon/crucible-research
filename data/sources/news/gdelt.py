...
class GdeltNewsAdapter:
    """GDELT 2.0 DOC API adapter. Implements :class:`NewsSource`.
...
def _build_query(ticker: str, company_name: str) -> str:
    ...
    catch both ('NVDA' headlines + 'Nvidia' headlines) and gate on
    US source country to filter noise."""
    # GDELT uses parenthesized OR + space-separated tokens with quoted
    # multi-word names. We always include US sourcecountry filter.
...
def _to_article(item: dict, *, ticker: str) -> NewsArticle | None:
    ...
    try:
        ...
        if not url:
            return None
        return NewsArticle(
            tickers=(ticker,),  # GDELT doesn't expose cross-ticker list per article
            title=item.get("title") or "",
            body_excerpt=item.get("seendate", "") and "",  # GDELT DOC API doesn't expose body
            url=url,
