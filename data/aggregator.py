...
Wave 1 PR A of the institutional data revamp.
...
Architecture:
    ...
    [Benzinga]──┘        ▼
                  fan-in concatenate
                         │
                         ▼
                   dedup pass (URL hash → title-similarity LSH)
                         │
                         ▼
...
class AggregatedNewsArticle:
    ...
    @property
    def total_trust_weight(self) -> float:
        # Computed at consumption time by joining variants to a trust
        # config; placeholder property to make the API clear.
        return float(self.n_sources)
...
def _normalize_title(title: str) -> str:
    ...
    cheapest dedup signal that catches the obvious cases. PR-A
...
class NewsAggregator:
    """Fan-in across :class:`NewsSource` adapters → dedup → trust weights.
    ...
    def trust_weight(self, source_name: str) -> float:
        if source_name not in self._trust_weights:
            logger.warning(
                "[aggregator] no trust weight configured for source %r — "
        ...
        *,
        ...
        for source in self._sources:
            try:
                ...
                logger.exception(
                    "[aggregator] source %r raised: %s — continuing with "
                    "remaining sources", source.name, e
                )
                continue
            all_articles.extend(batch)
            logger.info(
                "[aggregator] %s returned %d articles", source.name, len(batch)
            )
