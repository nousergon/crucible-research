...
def test_read_returns_payload_on_success():
    ...
    mock_s3.get_object.return_value = {
        "Body": MagicMock(read=lambda: json.dumps({"NVDA": {"sector": "Tech", "quality_score": 85.0}}).encode())
    }
    with patch("boto3.client", return_value=mock_s3):
        result = read_factor_profiles_from_s3(bucket="test-bucket")
...
class TestComputeAndWriteFactorProfilesSupplemental:
    ...
    def _mock_s3(self, *, supp_technical=None, supp_fundamental=None):
        ...
        # The fundamental group resolves latest-on-or-before via a prefix
        # listing rather than an exact key, so the double has to serve one.
        s3.list_objects_v2.return_value = {
            "CommonPrefixes": [{"Prefix": "features/2026-07-12/"}],
            "IsTruncated": False,
        }
        return s3

    def _patch_technical(self):
        """Technical inputs come from ArcticDB now, not the S3 partition.

        These tests are about the metron-supplemental CONCAT, so the daily
        technical read is stubbed with the same core frame the S3 parquet
        used to supply — keeping the assertions about supplemental merging
        intact rather than rewriting what they cover.
        """
        return patch(
            "scoring.factor_scoring._technical_frame_from_arctic",
            return_value=self._CORE_TECHNICAL.copy(),
        )

    def test_supplemental_absent_scores_only_core_universe(self):
        s3 = self._mock_s3()
        with patch("boto3.client", return_value=s3), self._patch_technical():
            compute_and_write_factor_profiles(
                run_date="2026-07-12",
...
class TestStewardshipInstitutionalWiring:
    ...
    def test_empty_inst_ownership_df_degrades_to_two_components(self):
        ...
        out = compute_factor_composites(
            ...
            fund,
            sector_map,
            inst_ownership_df=empty_inst_df,
        ).set_index("ticker")
        assert out["stewardship_score"].notna().all()
        assert (out["stewardship_n"] == 2).all()
