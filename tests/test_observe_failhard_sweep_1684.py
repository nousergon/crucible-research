...
def test_shadow_spec_failure_fires_observe_alert_and_is_non_fatal():
    ...
    with (
        ...
        out, errors = ss.build_shadow_artifacts(
    ...
    # (c) config#6428: the failure is ALSO recorded as an explicit MISS in
    # `errors`, not merely omitted from `out` — champion-challenger-policy.md
    # §3 ("a cycle where an arm produces no output is recorded as a miss, not
    # omitted"). Additive to (a)/(b), never a replacement for either.
    assert "boom_spec" in errors
    assert "kaboom" in errors["boom_spec"]
...
def test_healthy_shadow_spec_does_not_alert():
    ...
    with (
        patch.object(ss, "challenger_specs", return_value=[good]),
    ...
    # config#6428: a healthy cycle records no miss for the spec.
    assert errors == {}
