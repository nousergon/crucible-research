...
def create_quant_tools(context: dict) -> list:
    ...
    #     Empty set → no focus list available; override tagging is skipped.
    ...
    def get_factor_profile(tickers: list[str]) -> str:
        ...
        results: dict = dict(errors)
        for t in valid[:20]:
            # agent_override telemetry: tool-call on a ticker outside the
            # team's focus list = the agent reaching outside the curated
            # set. Append even on missing-profile (the intent was to
            # check) — archive_writer dedupes per-ticker. Skipped when
            # focus_list_tickers is empty (no focus list this cycle).
